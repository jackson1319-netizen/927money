...
# --- 3. IAT2 資料與核心函式 ---
# 基準數據：年繳保費 120,918 之 IAT2 解約金 (擷取自 PDF 試算表)
...
with st.sidebar:
    ...
    monthly_deposit = st.number_input("💵 月存金額", value=10076, step=1000) # 預設約年繳 12 萬
    st.divider()
    mode = st.radio("🔄 選擇策略模式", ["🛡️ 以息養險 (折抵保費)", "🚀 階梯槓桿 (複利滾存)"])
    st.info("💡 產品：新吉好利 (IAT2)\n繳費年期：6 年\n基金假設：年化 7% 配息/報酬")

# --- 5. 主畫面 ---
st.title("📊 IAT2 策略全能計算機 (6年期儲蓄版)")
...
for age in range(start_age + 1, start_age + 51):
    ...
    if "以息養險" in mode:
        ...
        row_display.update({
            ...
            "⑥保單借款": format_money(-current_loan),
            "⑦基金本金": format_money(current_fund),
            "⑧總淨資產": format_money(total_net_asset),
            "⑨身故金": format_money(total_death_benefit)
        })
        raw_data_rows.append({"loan_year": is_borrowing_year, "real_pay_val": actual_pay_yearly})
    else:
        ...
        raw_data_rows.append({"loan_year": is_borrowing_year})
...
st.warning("⚠️ 此試算為模擬 IAT2 產品特性之結果，實際數值應以建議書與保單條款為準。")
